...
if($result['Flag'] == 100){
	if($_GET['type'] == 'agency'){
		...
		ShowMsg('','http://'.$_SERVER['HTTP_HOST'].'/shop/recharge_success.php?style=baofoo&recharge_money='.$result['Money'].'&PayId='.$result['PayId'].'&ChannelId='.$result['ChannelId'].'&uin='.$result['Uin'].'&group_id='.intval($_GET['GroupId']));
	}
}elseif($result['Flag'] == 103){
	ShowMsg('','http://'.$_SERVER['HTTP_HOST'].'/shop/recharge_fail.php?style=baofoo&recharge_money='.$result['Money'].'&PayId='.$result['PayId'].'&ChannelId='.$result['ChannelId'].'&uin='.$result['Uin'].'&group_id='.intval($_GET['GroupId']).'&trade_id='.$result['TradeId']);
}else{
	echo $result['Result'];
}
