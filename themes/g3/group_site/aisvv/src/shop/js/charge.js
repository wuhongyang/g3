(function(){
	...
	$('#J_recharge').click(function(){
		...
		window.open ('index.php?module=recharge_new_window&group_id=' + groupId(), "_blank", "height="+screen.height+", width="+screen.width+", toolbar =no, menubar=no, scrollbars=no, resizable=no, location=no, status=no, scrollbars=yes");
		art.dialog({
			title: '提示',
			content: '完成充值后请点击确定查看资金是否到账，为他人充值请与对方确认资金是否到账',
			lock: true,
			button: [{
				name: "确定",
				callback: function(){
					location.href = '/service/account.php?module=vdianlist&group_id=' + groupId();
				},
				focus: true
			},{
				name: "关闭",
