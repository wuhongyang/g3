...
<dl class="body-side-list">
    ...
    <dd>
        ...
        <?php if(is_array($val['child'])) {foreach((array)$val['child'] as $val2) {?>
            <li>
            	<?php if(preg_match('/\/g\/([0-9]+)/',$val2['url'])) { ?>
                <a href="javascript:;" onclick="external.cpp_goSrc('<?php echo str_replace('http://'.$_SERVER['HTTP_HOST'],'',$val2['url']).'&vclient';;?>','<?php echo $val2['name'];?>')" title="<?php echo $val2['name'];?>">
                <?php } else { ?>
