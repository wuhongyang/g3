...
//------------------------------手机验证----------------------------------------
	...
	//注册验证码
	this.sendRegCode = function(phone){
		var Phone = phone;
		if(this.pregPhone.exec(Phone))
		{
			//this.showError(obj,'正确',1);
		...
		new Ajax().ajaxRequest('usersafe.php?module=sendcode','nouser='+phone,'post',
			function(rst){
				rst = (typeof JSON == 'object') ? JSON.parse(rst.responseText) : eval('('+rst.responseText+')');
				if(rst.Flag == 100){
					//把短信uniqueId记录下来
					var hidden = '<input type="hidden" name="uniqueid" value="' + rst.UniqueId + '">';
					$('input[name="uniqueid"]').remove();
					$('#phonesub').append(hidden);
					//改变按钮状态
					$("#getcode").unbind().html("已发送...请等待").attr("disabled", true);
					delDom();
				}else{
	...
	this.sendPwdCode = function(obj){
		...
		new Ajax().ajaxRequest('index.php?user_phone&sendcode','user='+this.inputPhone,'post',
			function(rst){
				if(rst.responseText == 1){
					obj.disabled = true;
					obj.value = '校验码已发送';
				}else if(rst.responseText == 2){
					alert('用户不存在');
				}else if(rst.responseText == 3){
					alert('发送太快,请稍等一会儿');
				}else{
					alert('发送失败');
				}
...
//-----------------------------其他表单验证-------------------------------------------
	...
	this.onsubmit = function(obj){
		for(i=0; i < obj.length; i++){
			if(obj[i].title == 'error') return false;
...
function showPage(total,perpage){
	...
	var prepage = parseInt(nowpage,10) - 1 < 1 ? 1 : nowpage;
