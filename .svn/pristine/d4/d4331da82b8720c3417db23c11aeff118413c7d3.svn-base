...
foreach($result as $key=>$val){
	...
	file_put_contents($_SERVER['DOCUMENT_ROOT'].'dengliang/http_load-12mar2006/test.txt',$url);
	exec("{$_SERVER['DOCUMENT_ROOT']}test/http_load-12mar2006/http_load -p {$paiallel} -f {$num} -s {$time} {$_SERVER['DOCUMENT_ROOT']}test/http_load-12mar2006/test.txt",$rst);
	$rst = implode('<br>',$rst);
	$sql = "INSERT INTO g3_test.test_httpload (bigcase_id,case_id,parent_id,child_id,result)VALUES
