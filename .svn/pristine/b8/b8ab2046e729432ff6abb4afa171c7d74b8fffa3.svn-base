 ...
 *文件: voucher_sys.class.php
 *copyright: (C) 2006-2011 by 杭州奥点科技有限公司
 */

class voucher_sys implements fund_factory
{
    ...
    /**
    * 构造函数
    * @param object $voucher_db 传入数据库操作对象
    */
    private function __construct(){
		$this->uptime = time();
        $this->today = strtotime(date('Y-m-d 00:00:00'));
        $this->voucher_db = domain::main()->GroupDBConn();
    ...
    */
    public function getLastBalance($info) {
        ...
        $sql = "SELECT last_balance FROM ".DB_NAME_VOUCHER.".voucher_balance WHERE uin={$info['uin']} AND group_id = {$info['group_id']} LIMIT 1 FOR UPDATE";
        $balance = $this->voucher_db->get_var($sql);
    ...
    * @return boolen|intger 返回余额如果余额为空返回NULL否则返回FALSE
    */
    public function getTaxBalance($info) {
        if(empty($info['group_id'])){
            return array('Flag'=>101,'FlagString'=>'参数不正确');
        }
        $sql = "SELECT last_balance FROM ".DB_NAME_TAX.".tax_parent_balance WHERE group_id = {$info['group_id']} AND parent_id = 10264  ORDER BY uptime DESC LIMIT 1";
        $balance = $this->voucher_db->get_var($sql);
        return array('Flag'=>100,'FlagString'=>'success','LastBalance'=>$balance);
    }
    ...
    */
    public function getBusinessBalance($info){
        ...
        if($bigcaseid >0 && $caseid>0 &&$parentid >0 && $groupid >0){
            $sql = "SELECT last_balance FROM ".DB_NAME_VOUCHER.".voucher_parent_balance WHERE group_id = {$groupid} AND parent_id={$parentid}  ORDER BY uptime DESC LIMIT 1";
            $balance = $this->voucher_db->get_var($sql);
            if($balance){
                return array('Flag'=>100,'FlagString'=>'success','LastBalance'=>$balance);
            }else{
    ...
    */
    public function trade($info){
		...
		if($child['trade_property'] == 5){
			$this->action();
			$sql = "SELECT uin,last_running_id,last_balance,balance_status,last_balance_check FROM ".DB_NAME_VOUCHER.".voucher_balance WHERE uin={$info['uin']} AND group_id={$info['group_id']} LIMIT 1 FOR UPDATE ";
			$account = $this->voucher_db->get_row($sql,'ASSOC'); //余额账户信息
			if(!empty($account)){
				...
				$sql = "INSERT INTO ".DB_NAME_VOUCHER.".voucher_balance (uin,group_id,last_balance,last_balance_check,balance_status) VALUES ({$info['uin']},{$info['group_id']},0,'{$check}',1) ";
				$query = $this->voucher_db->query($sql);
			}
	...
	private function VoucherTrade($info,$child){
		...
		//科目余额表
		$info['business_daytotal'] = $this->businessDayCount(DB_NAME_VOUCHER,'voucher_parent_balance',$info['trade_money'],$child,$info['group_id']);
 ...
 /**
    ...
    */
    private function updateLastBalance($account,$group_id,$money,$trade_type){
		...
		$sql = "UPDATE ".DB_NAME_VOUCHER.".voucher_balance SET last_balance={$money} ,last_balance_check='{$check}',uptime={$this->uptime} WHERE uin = {$uin} AND group_id = {$group_id} ";
        if(!$this->voucher_db->query($sql)){
            $this->rollback();
    ...
    */
    public function getTradeInfo($parent_id, $child_id) {
        ...
        $sql = "SELECT * FROM ".DB_NAME_VOUCHER_PLAT.".config WHERE parent_id={$parent_id} AND child_id={$child_id} AND child_status = 1";
        if($child_id > 900){
            $sql = "SELECT * FROM ".DB_NAME_VOUCHER_PLAT.".common_child_config WHERE  child_id={$child_id}";
        }
    ...
    private function checkOperator($operator){
        $opt = $this->plat_db->get_row("SELECT * FROM ".DB_NAME_VOUCHER_PLAT.".operator WHERE operator_key='{$operator}' AND operator_status=1 LIMIT 1");
        if(empty($opt)){
    ...
    */
	public function action(){
		$this->voucher_db->query('SET SESSION TRANSACTION ISOLATION LEVEL READ COMMITTED;');
        $this->voucher_db->start_transaction();
	}
