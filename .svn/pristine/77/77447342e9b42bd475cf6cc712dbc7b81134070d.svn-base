...
$param = array(
	'extparam' => array('Tag'=>'AlipayReturn','notOpenAgent'=>intval($_GET['notOpenAgent'])),
...
if($result['Flag'] == 100){
	if($_GET['type'] == 'agency'){
		...
		if(empty($result['Callback'])){
			ShowMsg('','http://'.$_SERVER['HTTP_HOST'].'/shop/recharge_success.php?style=alipay&recharge_money='.$result['Money'].'&PayId='.$result['PayId'].'&ChannelId='.$result['ChannelId'].'&uin='.$result['Uin'].'&group_id='.intval($_GET['GroupId']));
		}else{
			ShowMsg('','http://'.$_SERVER['HTTP_HOST'].'/vip.php?module=buy_success');
		}
	}
}elseif($result['Flag'] == 103){
	ShowMsg('','http://'.$_SERVER['HTTP_HOST'].'/shop/recharge_fail.php?style=alipay&recharge_money='.$result['Money'].'&PayId='.$result['PayId'].'&ChannelId='.$result['ChannelId'].'&uin='.$result['Uin'].'&group_id='.intval($_GET['GroupId']).'&trade_id='.$result['TradeId']);
}else{
	echo $result['Result'];
}
