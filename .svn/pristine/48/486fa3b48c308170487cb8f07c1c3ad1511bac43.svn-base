...
class kincome_sys implements fund_factory
{
    ...
    */
    public function getLastBalance($info) {
        ...
        $sql = "SELECT last_balance FROM ".DB_NAME_KINCOME.".".$this->table_pre."_balance WHERE uin={$info['uin']} AND group_id = {$info['group_id']} LIMIT 1 FOR UPDATE";
    ...
    */
    public function getBusinessBalance($info){
        ...
        if($bigcaseid >0 && $caseid>0 &&$parentid >0 && $groupid >0){
            $sql = "SELECT last_balance FROM ".DB_NAME_KINCOME.".".$this->table_pre."_parent_balance WHERE group_id = {$groupid} AND parent_id={$parentid}  ORDER BY uptime DESC LIMIT 1";
            $balance = $this->kwealth_db->get_var($sql);
            if($balance){
                return array('Flag'=>100,'FlagString'=>'success','LastBalance'=>$balance);
            }else{
    ...
    */
    public function trade($info){
		...
		if($child['trade_property'] == 5){
			$this->action();
			$sql = "SELECT uin,last_running_id,last_balance,balance_status,last_balance_check FROM ".DB_NAME_KINCOME.".".$this->table_pre."_balance WHERE uin={$info['uin']} AND group_id={$info['group_id']} LIMIT 1 FOR UPDATE ";
			...
			if(!empty($account)){
				...
				$sql = "INSERT INTO ".DB_NAME_KINCOME.".".$this->table_pre."_balance (uin,group_id,last_balance,last_balance_check,balance_status) VALUES ({$info['uin']},{$info['group_id']},0,'{$check}',1) ";
	...
	private function VoucherTrade($info,$child){
		...
		//科目余额表
		$info['business_daytotal'] = $this->businessDayCount(DB_NAME_KINCOME,$this->table_pre.'_parent_balance',$info['trade_money'],$child,$info['group_id']);
 ...
 /**
    ...
    */
    private function updateLastBalance($account,$group_id,$money,$trade_type){
		...
		$sql = "UPDATE ".DB_NAME_KINCOME.".".$this->table_pre."_balance SET last_balance={$money} ,last_balance_check='{$check}',uptime={$this->uptime} WHERE uin = {$uin} AND group_id = {$group_id} ";
