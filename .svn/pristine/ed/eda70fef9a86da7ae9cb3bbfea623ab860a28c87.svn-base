...
$result = httpPOST(GRIDFS_API_PATH,array('extparam'=>array('Tag'=>'GetFile','where'=>array('groupbanner'=>(int)$_GET['groupbanner']))));
if($result['Flag'] == 100){
...
$bytes = file_get_contents('group/banner1.jpg');
$thumb = new thumb($bytes);
$thumb->zoom($_GET['w'],$_GET['h']);
$thumb->display();
