...
if(!empty($id) && $id!=0){
	$param = array(
		'param' => array("BigCaseId"=>10002,"CaseId"=>10069,"ParentId"=>10648,"ChildId"=>110,"Desc"=>"分站礼物信息"),
		'extparam' => array("Tag"=>"FunctionPropsList","Id"=>$id)
	);
	...
	//分站礼物详情
	$info = $result['list'];
	$url = $link_array[110]['url'].'?module=props_update';
	...
	$param = array(
		'param' => array("BigCaseId"=>10002,"CaseId"=>10069,"ParentId"=>10648,"ChildId"=>110,"Desc"=>"分站礼物信息"),
		'extparam' => array("Tag"=>"FunctionPropsList")
	);
	$result = request($param);
	//得到图片分类
	$cat = httpPOST(GRIDFS_API_PATH,array("extparam"=>array('Tag'=>'CatList','mypost'=>array('state'=>1))));
	...
	$areaInfo = $result['region'];
	$areaInfo = json_encode($areaInfo);
	$url = $link_array[110]['url'].'?module=props_add';
}

$tpl = template::getInstance();
$tpl->setOptions(get_config('template','admin'));
include template('regions/function_props_info.html',$tpl);