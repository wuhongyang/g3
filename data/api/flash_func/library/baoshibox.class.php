...
* @copyright (c) 奥点科技
*/
class Baoshi extends AbstractClass
{
	protected $db;
	const PAY_KMONEY   = 101; //V豆支付
	const STORE_KMONEY   = 102; //V豆存入(接收)
	const PRIZEUSER_KMONEY   = 103; //V豆存入(用户中奖)
	...
	*/
	 ...
	 * @return [type]           [送礼结果]
	 */
	public function deduct($param,$extparam){
		...
		if($prize_money > 0){
			...
			$desc = $user.'本次赠送'.$gift_info['info']['parent_name'].'共中奖'.$j.'次,中奖总金额'.$prize_money.',赠与'.$param['TargetUin'].':'.$targetprize;
